...
-> 게시글에 필요한 데이터들로, HTML 수정/등록 폼에서 입력받은 값들이 저장되는 변수들이다. 이 클래스는 요청 데이터(DTO) 역할을 한다.

-> DTO를 Entity로 변환, 빌더 패턴으로 객체를 생성하였다. 폼에 입력된 값들을 DTO에 담고 DTO를 Entity로 변환하여 DB에 저장한다.

...
- 스프링 부트 동작 과정에 대해서 배웠고 내부 컨테이너 전체 동작 과정에 대해서도 알게되었음음
- Thymeleaf 문법 -> 텍스트 출력: <span th: text="${data}">, $<...> 변수 표현식
...
- 개인 포트폴리오 템플릿 -> 포트폴리오 홈페이지 제작
- 나의 전공/세부 분야, 프로필 수정, 상세페이지 수정
...
- 프로필 수정(기술&경험)
- Thymeleaf 문법 -> 텍스트 출력 html 태그 인식(th:utext), Input/select 등 태그의 값 지정(th:value,th:field), 태그의 속성 값 여러 개 지정(링크는 @)(th:href)
- 지역변수(태그 내에서만 동작)(th:with), 링크에 파라미터 추가하기(&를 ,로 표현)
- JPA 역할 및 특징 -> 데이터 제어와 영속성 제공/ JPA의 대표 구현체(ORM)은 하이버네이트(Hibernate)
- 데이터베이스 연동 확인
- 구조 변경 -> View/ Controller에는 controller/ model에는 service, repository, domain 폴더 생성/ DTO 계층간 데이터 교환
...
- CRUD Operation 동작(주요 메서드 기능)/ HTTP Method 표준: POST,GET,PUT(PATCH),DELETE
- 블로그 게시판 작성 -> article_list.html 생성
- 빌더 패턴: 객체 인스턴스를 생성하는 패턴
- DI 의존성 주입: @Service, @Controller, @Repository -> 컨테이너에 bean으로 자동 등록/ 생성자, 일반필드에서 주입할 수 있음
...
- ORM: 객체와 관계형 데이터를 매핑하기 위한 기술, 복잡한 SQL 쿼리를 직접 개발할 필요없음
- POST 요청 -> 왜 PUT 요청을 사용해야 하나: POST는 다수 요청에 대해 다수 게시글이 생성됨/ PUT은 다수 요청에 대해 동일한 결과, 동일 데이터 수정
- spring.mvc.hiddenmethod.filter.enabled=true -> application.properties 설정 파일을 수정, HTML 기본 폼 전송은 GET/POST 두 가지만 지원/ put, delete 지원하지 않았음
...
- 프로필 수정(나의 프로젝트 소스코드)
- board_list(게시판) 생성/ 사용자, 날짜, 조회수, 좋아요 필드 추가
- BoardRepository 리포지토리 연동
- board_view(글 내용 보기 페이지) 생성 <-board_list.html 재활용
...
- 캐싱 서비스 지원: 자주 사용하는 데이터를 저장하여 재활용-> Ehcache(로컬), Redis(외부:분산)
- findByTitleContaining 메서드: 단순 데이터 셋 검색에 적합, like 검색 가능, 대규모 검색 엔진에는 적합X
- 페이징: 데이터를 페이지 단위로 나눠서 처리/ 주요 페이지 기능 제공/ 내부 기본값 설정 -> page 0, size 20, 정렬 X, @PageableDefault
- findByTitleContainingIgnoreCase -> 대소문자 무시, like 검색 기능
...
- 스프링 시큐리티 -> 내부 동작: 인증(누구), 인가(위임)/ Security 정보를 전용 Context에 저장
- 암호화 알고리즘 -> Bcrypt 제공(단방향 암호화)
- 필터체인, 암호화 설정 등
- Unique 속성은 중복X
- 패스워드 encode 부분 설정(인코딩)
...
- 스프링 세션 -> 스프링 부트 환경의 세션 관리, 세션 쿠키: 임시, 일회성/ 세션ID를 쿠키로 저장, 서버 부하 적음
- 스프링 부트 내부 보안 필터(다양한 보안 필터 존재)
- commitSession -> JSESSIONID 값 생성/ JSESSIONID -> 쿠키에 저장된 문자열, 서버 기반 세션 식별자
- 프로필 수정(나의 팀원/ 추천글, 후기)
- 로그인 과정에 세션 생성
- invalidate()-> 세션 무효화, setPath -> 쿠키의 경로, setMaxAge(0) -> 쿠키 만료 0이면 삭제
- .maximumSessions() 사용자별 세션 최대수/ .maxSessionsPreventsLogin(true) -> 동시 세션 제한
...
- toAbsolutePath() -> 절대 경로로 바꿈, resolve -> 폴더에 하위 경로나 파일명을 붙여서 새로운 경로를 만드는 기능
- catch안에는 예외 처리
